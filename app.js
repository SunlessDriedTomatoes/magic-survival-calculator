...
// applies to. Verified two ways: (1) every one of the 58 fusions whose own effect text names a
// spell (e.g. "Increase Flash Shock Damage by 7X") names exactly the spell behind
// requiresEvolutionIds[0], never [1], with zero exceptions across the full dataset; (2) directly
// confirmed by you for Frenzy → Berserk (Satan's Nails/Flash Shock is requiresEvolutionIds[0];
// Fire at Will/Magic Bolt, requiresEvolutionIds[1], gets none of its bonuses). A [0] slot can also
// be a maxed Passive rather than a spell evolution (e.g. Overmind's is Intelligence) — those
// fusions grant no spell-specific damage at all (global buffs only, handled separately), so
// returning null for them is correct: there's no "main spell" to gate an Ultimate on.
...
// Shield and Cloaking are the two confirmed exceptions to "base damage always comes straight from
// the in-game encyclopedia" — the extracted data had no damage figure for either (both showed only
// a "no direct damage stat" utility note), but the real encyclopedia does list one for each (30 and
// 500 respectively, confirmed directly), now baked into base_spell_stats.json like every other
// spell. Both are per-hit, matching this calculator's convention everywhere else. Neither spell
...
// pools everything else uses. Unlike the ultimates (cross-checked against 20+ real reported
// numbers), there's no equivalent validation data for these two, so this is a best-effort literal
// reading of their text, flagged as such wherever it's shown.
...
// x1.25"), so the x1.25 is hardcoded here rather than parsed, same confidence caveat as Titan/Nexus.
...
// normal-tier version of the same passive. Unlike the normal tier (confirmed non-uniform, 2-5 per
// passive), the post-max cap is meant to be uniform across all of them — no extracted data
// confirms the exact number, so this uses 7 per your own recollection, flagged in the UI as
// unconfirmed (a "Genius" synergy extends it by +10, implying a real base cap exists somewhere).
...
// unlocks one level later. Verified directly against in-game screenshots for School classes
// (confirmed 4 levels total; Wizard at level 1 shows lines 1-2 active, at level 3 shows 1-4).
...
// genuinely separate "Magic Bolt Lv+1" grants down to one, hiding a real bonus the in-game tooltip
// clearly shows as two active lines at level 1 (confirmed against a screenshot). Since neither case
// maps to a tracked damage/stat bucket anyway (both fall through classifyEffect to 'other'), the
// safer default is to show everything the data actually contains rather than guess which
// same-text pairs are "real" duplicates.
...
function compute() {
  ...
  // Base Crit Chance (5%) and Base Crit Multiplier (200%) are fixed, confirmed values with zero
  // bonuses. Everything found above (Critical Strike Rate/Multiplier effects) stacks additively
  // on top, matching how every other %-based pool in this calculator works.
  ...
  if (ARCHMAGE_CLASS && state.classId === ARCHMAGE_CLASS.id) {
    ...
    // restated, so this may currently double-count Archmage specifically. Flagged, not yet verified
...
// Artifact preview), so destructive actions use an inline "are you sure" toggle instead.
// Native confirm()/alert() can be blocked or unreliable inside a sandboxed iframe (e.g. an
...
function renderLeftPane() {
  ...
  // Crit Chance/Multiplier base values are fixed (confirmed 5%/200% with zero bonuses) — not
  // player-editable, same treatment as Base ATK. Every "Increase Critical Strike Rate/Multiplier
  // by X%" effect among current selections is found automatically and added on top.
  const critR = compute();
